...
def calculate_recommendations(user_ids, df_events):
    if df_events.empty:
    ...
    for uid in user_ids:
        ...
        top_recs = recommendations.groupby(level=0).mean().sort_values(ascending=False).head(5)
        
        results[str(uid)] = [
            {"sku": int(pid), "score": float(score)} 
